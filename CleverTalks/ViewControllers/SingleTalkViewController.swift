...
final class SingleTalkViewController: MessagesViewController {
    ...
    private func presentLocationPicker() {
        ...
        vc.completion = { [weak self] selectedCoorindates in
            ...
            DatabaseManager.shared.sendMessage(to: talkId, otherUserEmail: strongSelf.otherUserEmail, name: name, newMessage: message, completion: { success in
                if success {
...
extension  SingleTalkViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
...
extension SingleTalkViewController: MessagesDataSource, MessagesLayoutDelegate, MessagesDisplayDelegate {
...
extension SingleTalkViewController: MessageCellDelegate {
        ...
        if isNewTalk {
            ...
            DatabaseManager.shared.createNewTalk(with: otherUserEmail, name: self.title ?? "User", firstMessage: message, completion: { [weak self]success in
                if success {
                    ...
                    let newTalkId = "talk\(message.messageId)"
                    self?.talkId = newTalkId
                    self?.listenForMessages(id: newTalkId, shouldScrollToBottom: true)
            ...
            DatabaseManager.shared.sendMessage(to: talkId, otherUserEmail: otherUserEmail, name: name, newMessage: message, completion: { [weak self] success in
                if success {
                    self?.messageInputBar.inputTextView.text = nil
                    print("message sent")
                }
